...
- 当前目录**按文件名搜索/过滤**（工具栏输入框，前端过滤，不发起新的列举请求；切换目录自动清空）
- 点击列头**排序**（名称 / 大小 / 修改时间），文件夹始终置顶，与常见文件管理器习惯一致
...
- 支持将文件或**整个文件夹**拖拽到表格区域直接加入上传队列（悬停显示遮罩提示）；
  文件夹会递归展开并保留原有子目录结构，空文件夹会在目标目录下创建对应的占位文件夹
...
- **打包下载（zip）**：单个目录的行内菜单，或多选后在工具栏批量将文件/目录打包为一个 zip 下载；
  过程分「列举 → 逐个下载 → 生成 zip」三阶段展示阻塞式进度，含桶根「回收站」时禁用
