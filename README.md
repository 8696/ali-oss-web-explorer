...
| 层级 | 职责 |
|------|------|
| **`App.tsx`** | 组合 `useOSSConfig` / `useOSSClient` / `useOSSFiles` / `useUploadTasks`，连接各子组件与弹窗，处理跨模块的交互（如删除后更新多选 key、上传批次结束统一刷新列表）。 |
| **`hooks/`** | 配置持久化、客户端创建与连接探测、当前目录与列表（含 URL 同步）、上传队列与并发控制。 |
| **`services/oss.ts`** | **唯一**与 `ali-oss` 直接交互的模块：列举、上传、删除/回收站备份、重命名（copy+delete）、签名 URL、对象 ACL 等；并对浏览器端常见错误（如 CORS 未暴露 ETag）做中文提示归一化。 |
| **`components/`** | 纯 UI 与局部交互：工具栏、表格、面包屑、各类 Drawer/Modal。 |
| **`constants/`** | 与业务约定相关的常量（如地域列表、桶根「回收站」目录规则）。 |
| **`types/`** | 领域模型与 `OSSConfig`、`FileEntry` 等类型，供全项目复用。 |
| **`utils/`** | `localStorage` 读写、路径/大小/日期格式化等无 OSS 依赖的工具。 |

数据流简述：用户填写配置 → `useOSSConfig` 写入本地存储 → `useOSSClient` 创建 client 并 `list` 探测 → `useOSSFiles` 按当前 `prefix` 拉取列表；表格与工具栏的操作经 `App` 回调进入对应 Hook 或 `services/oss`。
...
- **重命名**：支持文件与目录；目录重命名在 OSS 侧为复制前缀树再删除旧前缀，界面可展示**进度**（大目录耗时较长）。
...
├── types/
│   └── oss.ts                 # OSSConfig、FileEntry、上传任务等类型
└── utils/
    ├── storage.ts
    └── format.ts
```
...
1. 启动后点击工具栏 **连接配置**（或等价入口），填写 AccessKey、Bucket、Region，**连接**。
